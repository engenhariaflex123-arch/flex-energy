...
const Sidebar: React.FC<Props> = ({ open, clienteAtivo }) => {
  ...
  const navItem = (icon: string, label: string, active = false, onClick?: () => void, arrow?: string) => (
    <div onClick={onClick} style={{ display:'flex', alignItems:'center', gap:10, padding:'0.6rem 1.25rem', fontSize:15, color: active ? cores.laranja : cores.text2, cursor:'pointer', borderLeft: active ? `3px solid ${cores.laranja}` : '3px solid transparent', background: active ? cores.laranjaGlow : 'transparent' }}>
      <span>{icon}</span><span style={{flex:1}}>{label}</span>{arrow && <span style={{fontSize:12}}>{arrow}</span>}
    </div>
  );
  return (
    <div className="app-sidebar" style={{ position:'fixed', left:0, top:0, bottom:0, width:220, background:cores.bg2, borderRight:`1px solid ${cores.border}`, display:'flex', flexDirection:'column', zIndex:100, overflowY:'auto' }}>
      <div style={{ padding:'1.25rem', borderBottom:`1px solid ${cores.border}` }}>
        ...
        <div style={{ fontSize:12, color:cores.text3, marginTop:2 }}>Monitoramento Solar</div>
      </div>
      <div style={{ padding:'0.875rem 1.25rem', borderBottom:`1px solid ${cores.border}`, background:cores.laranjaGlow }}>
        <div style={{ fontSize:12, color:cores.laranja, textTransform:'uppercase', letterSpacing:'0.08em' }}>Cliente</div>
        <div style={{ fontSize:15, fontWeight:600, marginTop:2, color:cores.text }}>{usinaAtual?.nome || 'Carregando...'}</div>
        <div style={{ fontSize:13, color:cores.text3, marginTop:1 }}>📍 {usinaAtual?.cidade || ''}{usinaAtual?.estado ? `, ${usinaAtual.estado}` : ''}</div>
      </div>
      {grupoId && (
        <div
          onClick={() => navigate('/visao-geral')}
          style={{ display:'flex', alignItems:'center', gap:8, padding:'0.7rem 1.25rem', fontSize:14, color:cores.text2, cursor:'pointer', borderBottom:`1px solid ${cores.border}` }}
        >
          <span>←</span><span>Voltar para Visão Geral</span>
        </div>
      )}
      <nav style={{ flex:1, paddingTop:'0.5rem' }}>
        <div style={{ padding:'0.5rem 1.25rem 0.25rem', fontSize:12, color:cores.text3, textTransform:'uppercase', letterSpacing:'0.08em' }}>Visão Geral</div>
        {navItem('⚡','Dashboard', location.pathname === '/dashboard', () => irPara('/dashboard'))}
        {navItem('🔎','Análise', location.pathname === '/analise', () => irPara('/analise'))}
        {navItem('🔔','Alarmes')}
        <div style={{ padding:'0.5rem 1.25rem 0.25rem', fontSize:12, color:cores.text3, textTransform:'uppercase', letterSpacing:'0.08em' }}>Consumo</div>
        ...
        {consumoOpen && ['Geral (Principal)','Ramal — Produção','Ramal — Administrativo','Ramal — Ar Condicionado'].map((r,i) => (
          <div key={i} style={{ padding:'0.4rem 1.25rem 0.4rem 3rem', fontSize:14, color: i===0 ? cores.laranja : cores.text3, cursor:'pointer' }}>{i===0?'●':'○'} {r}</div>
        ))}
        <div style={{ padding:'0.5rem 1.25rem 0.25rem', fontSize:12, color:cores.text3, textTransform:'uppercase', letterSpacing:'0.08em' }}>Geração</div>
        ...
        <div style={{ padding:'0.5rem 1.25rem 0.25rem', fontSize:12, color:cores.text3, textTransform:'uppercase', letterSpacing:'0.08em' }}>Análise</div>
      ...
      <div style={{ padding:'1rem 1.25rem', borderTop:`1px solid ${cores.border}`, display:'flex', alignItems:'center', gap:10 }}>
        <div style={{ width:32, height:32, borderRadius:'50%', background:cores.laranja, display:'flex', alignItems:'center', justifyContent:'center', fontSize:15, fontWeight:700, color:'#fff' }}>
          {usinaAtual?.nome?.charAt(0) || 'G'}
        </div>
        <div>
          <div style={{ fontSize:14, fontWeight:500, color:cores.text }}>{usinaAtual?.nome || 'Carregando...'}</div>
          <div style={{ fontSize:12, color:cores.text3 }}>Visualizador</div>
        </div>
      </div>
      <div style={{ padding:'0.5rem 1.25rem 1rem', borderTop:`1px solid ${cores.border}`, display:'flex', flexDirection:'column', gap:4 }}>
        <div
          onClick={toggleTheme}
          style={{ display:'flex', alignItems:'center', gap:8, padding:'0.5rem 0', fontSize:14, color:cores.text2, cursor:'pointer' }}
        >
        ...
        <div
          ...
          style={{ display:'flex', alignItems:'center', gap:8, padding:'0.5rem 0', fontSize:14, color: verificando ? cores.text3 : cores.text2, cursor: verificando ? 'default' : 'pointer' }}
        >
        ...
        <div
          ...
          style={{ display:'flex', alignItems:'center', gap:8, padding:'0.5rem 0', fontSize:14, color:'#F87171', cursor:'pointer' }}
        >
          <span>🚪</span>
          <span>Sair</span>
        </div>
